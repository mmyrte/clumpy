...
class CramerMRMR:
    ...
    def gof_Z(self, z, w, k, bound="none"):
        ...
        if self.kde_method:
            df = self.get_gof_df_kde(z=z, w=w, k=k, bound=bound)
    ...
    def compute_bins_1d(self, z, w, k):
        ...
        if self.approx == "mean":
            ...
            raise (
                ValueError(
                    "Unexpected 'self.approx' attribute value. It should be 'median' (default), 'mean', or 'std'"
                )
            )
        ...
        if Gamma < 10:
            raise (
                ValueError(
                    "The number of bins (Gamma) is too low. Increase the epsilon parameter."
                )
            )
        ...
        delta = Delta / Gamma
        self._1d_bins[k] = np.linspace(
            np.min(z[w]), np.max(z[w]) + 10**-5 * delta, Gamma
        )
    ...
    def gof(self, df, k):
        ...
        # chi2
        chi2 = (
            (df.loc[df["keep"], "O"] - df.loc[df["keep"], "E"]) ** 2
            / df.loc[df["keep"], "E"]
        ).sum()
        ...
        if Gamma - 1 <= 0:
            print(
                "Warning, the bins are too small for this transition.\nEventually increase the epsilon parameter."
            )
            return np.nan
    ...
    def toi_Z(self, Z, k0, k1, bounds):
        ...
        if self.kde_method:
            df = self.get_toi_df_kde(Z=Z, k0=k0, k1=k1, bounds=bounds)
    ...
    def compute_bins_2d(self, Z, k0, k1):
        ...
        if self.approx == "mean":
            ...
            for k, z in enumerate(Z.T):
                ...
                rho = kde.predict(y[:, None])
                Gamma.append(
                    np.max((10, int((n / n_m) ** 0.5 / 2 * np.max(rho) * Delta[k])))
                )
            ...
            for k, z in enumerate(Z.T):
                Gamma.append(
                    np.max((10, int((n / n_m) ** 0.5 * Delta[k] / np.std(z) / 2)))
                )
    ...
    def get_toi_df(self, Z, k0, k1):
        G = np.vstack(
            [np.digitize(z, bins=self._2d_bins[(k0, k1)][k]) for k, z in enumerate(Z.T)]
        ).T
        ...
        df_N = [
            G_df.groupby("g" + str(k)).size().reset_index(name="N" + str(k))
            for k in [k0, k1]
        ]
        for df_Nk in df_N:
    ...
    def toi(self, df, k0, k1):
        ...
        n = df.loc[df["keep"], "O"].sum()
        Gamma = [
            len(np.unique(df.loc[df["keep"], "g" + str(k)].values)) for k in [k0, k1]
        ]
    ...
    def mrmr_cramer(self, Z, w, bounds=None):
        ...
        if len(bounds) != d:
            raise (
                ValueError(
                    "Unexpected 'bounds' parameter. It should be a list of bounds for each explanatory variable or 'None'. A bound can be {'none', 'left', 'right', 'both'}."
                )
            )
        ...
        for k in range(d):
            ...
            if Z[:, k].std() == 0:
                ...
                V = self.gof_Z(z=Z[:, k], w=w, k=k, bound=bounds[k])
            V_gof.append(V)
        ...
        for k0, k1 in list_k0_k1:
            if k0 in evs and k1 in evs:
                ...
                V_toi = self.toi_Z(
                    Z=Z[:, [k0, k1]][w], k0=k0, k1=k1, bounds=[bounds[k0], bounds[k1]]
                )
    ...
    def tex_table(self, feature_names):
        ...
        tab += (
            "\\multicolumn{5}{Sc}{\\textsc{Test of Relevance}, $V_{GoF}^m="
            + str(self.V_gof_min)
            + "$}\\\ \n"
        )
        ...
        if len(self._V_toi.keys()) > 0:
            tab += (
                "\\multicolumn{5}{Sc}{\\textsc{Test of Redundancy}, $V_{ToI}^M="
                + str(self.V_toi_max)
                + "$}\\\ \n"
            )
    ...
    def subplot_gof(self, k):
        ...
        plt.text(
            ...
            + "$\n$R_{mean}="
            + str(round(self._R_mean_gof[k], 3))
            + "$\n$R_{max}="
            + str(round(self._R_max_gof[k], 3))
            + "$\nexluded pixels: "
            + str(round(self._excluded_gof[k] * 100, 2))
            + "%",
        )
    ...
    def plot_toi(
        self, k0, k1, xlim=None, ylim=None, reorder=False, path=None, cmap="plasma"
    ):
        ...
        if reorder:
            ...
            shape = (
                df["g" + str(k0)].values.max() + 1,
                df["g" + str(k1)].values.max() + 1,
            )
        ...
        plt.bar(
            df_sorted.loc[df_sorted["keep"]].index,
            df_sorted.loc[df_sorted["keep"], "E"],
            align="edge",
            width=1,
            alpha=0.8,
            color="darkviolet",
        ...
        plt.bar(
            df_sorted.loc[df_sorted["keep"]].index,
            df_sorted.loc[df_sorted["keep"], "O"],
            align="edge",
            width=1,
            alpha=0.8,
            color="orange",
        ...
        plt.text(
            ...
            + "$\n$R_{mean}="
            + str(round(self._R_mean_toi[(k0, k1)], 3))
            + "$\n$R_{max}="
            + str(round(self._R_max_toi[(k0, k1)], 3))
            + "$\nexluded pixels: "
            + str(round(self._excluded_toi[(k0, k1)] * 100, 2))
            + "%",
        )
