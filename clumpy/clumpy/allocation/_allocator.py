...
class Allocator:
    ...
    def __init__(
        self, calibrator: Calibrator = None, verbose=0, verbose_heading_level=1
    ):
    ...
    def nb_monte_carlo(
        ...
        J = lul_origin.get_J(state=initial_state, mask=mask)
        X = lul_origin.get_X(J=J, features=features)

        X = self.calibrator.feature_selector.transform(X)

        P, final_states, P_Y = self.calibrator.tpe.transition_probabilities(
            J=J, Y=X, P_v=P_v, return_P_Y=True, return_P_Y__v=False
        )
    ...
    def _clean_proba(self, P, final_states):
    ...
    def _sample_pivot(self, J, P, final_states, shuffle=True):

        P, final_states = self._clean_proba(P=P, final_states=final_states)

        V = generalized_allocation_rejection_test(P, final_states)
        ...
        if shuffle:
            n = J_pivot.size
            id_shuffled = np.random.choice(a=n, size=n, replace=False)
...
def _update_P_v__Y_u(P_v__u_Y, tm, inplace=True):
    ...
    P_v__u_Y[:, multiply_cols] *= tm.M[0, multiply_cols] / P_v__u_Y_mean[multiply_cols]
