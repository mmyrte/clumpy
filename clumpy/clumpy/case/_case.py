...
from .. import EVLayer, open_layer
...
class Case:
    ...
    def __init__(self, regions=None, palette=None, verbose=1):
    ...
    def get_ev_selector(self, region_info, land_state, final_state):
            ...
            for land in lands_list:
                if self.verbose > 0:
                    print(
                        ...
                        + str(land.state)
                        + " - "
                        + self.palette.get(land.state).label
                    )
                    print("---------\n")

                # data
                J, W, Z = get_J_W_Z(
                    initial_luc_layer=initial_luc_layer,
                    final_luc_layer=final_luc_layer,
                    evs=evs,
                    regions_layer=regions_layer,
    ...
    def allocate(
        self, luc_layer, evs, P_v, regions_layer=None, region_only=None, land_only=None
    ):
