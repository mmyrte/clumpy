...
class Bayes(TransitionProbabilityEstimator):
    ...
    def fit(self, Z, W, bounds=None, **kwargs):
        """
        ...
        for i, w in enumerate(W.T):
            if w.sum() > 0:
                ...
                if Z_fit.shape[0] > self.n_fit_max:
                    Z_fit = Z_fit[
                        np.random.choice(
                            a=Z_fit.shape[0], size=self.n_fit_max, replace=False
                        )
                    ]
    ...
    def compute(
        ...
        **kwargs,
        ...
        P_v__Y = self.bayes_adjustment(P_Y__v=P_Y__v, P_Y=P_Y, P_v=P_v)
    ...
    def bayes_adjustment(self, P_Y__v, P_Y, P_v):
        ...
        if np.sum(s > 1) > 0:
            if self.verbose > 0:
                ...
                print(
                    ...
                    + str(s.size)
                    + " ("
                    + str(np.round(s_sum / s.size * 100, 2))
                    + " %) uncorrect probabilities have been detected."
                )
            ...
            while np.sum(s > 1) > 0 and n_corrections < self.n_corrections_max:
                ...
                if self.log_computations == False:
                    # if no log computation
                    P_v__Y[id_anomalies] = (
                        P_v__Y[id_anomalies] / s[id_anomalies][:, None]
                    )

                    P_v__Y_mean = P_v__Y.mean(axis=0)
                    multiply_cols = P_v__Y_mean != 0
                    P_v__Y[:, multiply_cols] *= (
                        P_v[multiply_cols] / P_v__Y_mean[multiply_cols]
                    )
                    ...
                    # with log computation
                    log_P_v__Y[id_anomalies] = log_P_v__Y[id_anomalies] - np.log(
                        s[id_anomalies][:, None]
                    )
        ...
        id_anomalies = s > 1
        P_v__Y[id_anomalies] = P_v__Y[id_anomalies] / s[id_anomalies][:, None]
    ...
    def compute_P_Y(self, Y):
        ...
        if Y_fit.shape[0] > self.n_fit_max:
            Y_fit = Y_fit[
                np.random.choice(a=Y_fit.shape[0], size=self.n_fit_max, replace=False)
            ]
